...
def cost_to_go(start,goal):
    """ returns cost to go from current to goal node"""
    x1 = start[0]
    x2 = goal[0]
    y1 = start[1]
    y2 = goal[1]
...
def draw_arrow(ax,x1,y1,x2,y2):
    """ draw arrow representing move to new node"""
...
def check_viableX(point):
    """ returns true if the value of x is within the map, else false"""
...
def plot_workspace(x_start,y_start,x_goal,y_goal):
    """ plots the workspace in matplotlib"""
    fig, ax = plt.subplots()
...
def down_30(point, radius, clearance,orientation):
    """move forward at 30 degrees down"""
    x = point[0]
    y = point[1]
    cost = step_size
...
def down_60(point, radius, clearance,orientation):
    """move forward at 60 degrees down"""
    x = point[0]
    y = point[1]
    cost = step_size
...
parent, final_node = a_star(fig,ax, robot1)
plt.pause(1)
